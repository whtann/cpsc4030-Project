...
d3.csv("HeatMap.csv").then(function(dataset) {
    ...
    console.log(heatDiv.clientHeight)
    // append the svg object to the body of the page
    var svg = d3.select("#heatmap")
        .style("width", heatDimensions.width)
        .style("height", heatDimensions.height)
        .append("svg")
            .attr("width", heatDimensions.width)
            .attr("height", heatDimensions.height)
    ...
    var x = d3.scaleBand()
    .range([0, heatDimensions.width])
    .domain(myGroups)
    .padding(0.05);
    svg.append("g")
    .style("font-size", 15)
    .attr("transform", "translate(0," + heatDimensions.height + ")")
    .call(d3.axisBottom(x).tickSize(0))
    .select(".domain").remove()

    // Build Y scales and axis:
    var y = d3.scaleBand()
    .range([heatDimensions.height, 0 ])
    .domain(myVars)
    .padding(0.05);
    svg.append("g")
    .style("font-size", 15)
